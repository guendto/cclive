...
For example, "URL shorteners" (or "URL shortening services")
typically require following an HTTP redirection to another location.
If used, cclive stops working with such services.

Available only with quvi 0.2.16 and later. This options replaces
(and deprecates) C<--no-shortened>.

=item B<-s, --no-shortened>

Do not "decompress" shortened URLs before using them. Available only
with quvi 0.2.8 and later. See also C<--no-resolve> which replaces this
option.
...
is "%t.%s". The following specifiers can be used in the I<arg>:
...
A fancy way to feed cclive an URL.
...
Please note that each URL must be separated with a newline or a
...
You can use "g" for global (find all), e.g. if you wanted to replace
all of the "Alice" occurences with "Malice", you would use
"s{Alice}E<lt>MaliceE<gt>g".

=item B<cclive "URL" -b --log-file my.log>

Go to background, redirect output to C<my.log>.

=item B<kill -USR1 $cclive_pid>

Interrupt the current download (of $cclive_pid). Move to the next video
in the queue (if any).
...
=item B<Switch --no-download>

Use the C<--no-download> switch when you don't need to download the video.

=item B<Switch --verbose-libcurl>
...
Use C<strace(1)>, C<gdb(1)> and C<valgrind(1)>.
