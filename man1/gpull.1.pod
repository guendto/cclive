...
Disable all use of HTTP proxy. http_proxy setting will be ignored.
...
Use I<arg> for HTTP proxy, e.g. "http://foo:1234".

=item B<--throttle>=I<arg>

Limit download transfer rate to I<arg> KB/s.
...
Make sure you compile cclive with C<-g> (see C<g++(1)>) for debug symbols.
See C<Other tools>.
...
HOWTOs, typically from the $prefix/share/quvi and $prefix/share/doc/quvi
...
If you have cooked up a patch, you can submit it to the trackers (see L</BUGS>).
