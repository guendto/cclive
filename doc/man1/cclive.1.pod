...
=head1 OPTIONS - COMMAND LINE

These options may be specified on the command line only.
See also L</OPTIONS - CONFIGURATION>.
...
See also L</FILES> and C<--config-file>.

=head2 -r, --no-resolve  (no-resolve={true|false})

Do not resolve URL redirections. If enabled, breaks the compatibility
with most "shortened" URLs.

=head2 -c, --continue  (continue={true|false})

Resume partially downloaded media.

=head2 -p, --prefer-format I<arg>  (prefer-format=ARG)
...
=head2 --progressbar I<arg> [=normal]  (progressbar=ARG)
...
=head2 --output-dir I<arg>  (output-dir=ARG)

Write downloaded media to I<arg> directory.

=head2 -t, --tr I<arg>  (tr=ARG)
...
=head2 --transfer-timeout I<arg> [=0, disabled]  [transfer-timeout=ARG]
...
=head2 --dns-cache-timeout I<arg> [=60]  (dns-cache-timeout=ARG)
...
=head2 --max-retries I<arg> [=5, 0=disabled]  (max-retries=ARG)

Retry downloading I<arg> times before giving up.

=head2 --retry-wait I<arg> [=5]  (retry-wait=ARG)
...
This location is read by default. C<--config-file> may be used to specify
a different location. See also L</OPTIONS - CONFIGURATION>.
...
cclive exits with EXIT_SUCCESS (on POSIX systems this is 0) on success
and EXIT_FAILURE (on POSIX systems this is 1) if an error occurred.
