...
The I<arg> value is used with all of the URLs fed to cclive at runtime. If
...
to C<default> format if the I<arg> was unrecognized or the requested
...
The I<arg> is also invoked if the media is fully retrieved already. See
also L</EXAMPLES>.
...
=item cclive "URL" --exec 'echo "%f"'

Invoke C<echo(1)> after downloading the media. In this case, print the
