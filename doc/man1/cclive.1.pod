...
Query available formats to an URL. Each format string is separated with
a pipe character. To use a format string from the list, choose one and
use it with C<--format>.

libquvi creates dynamically the list from the data returned by the server.
The format strings were previously, until libquvi 0.2.16.1+, hardcoded
values that were I<assumed> to be available to an URL.

It should be noted that the returned list does not include the static format
strings C<default> and C<best>. They are presumed to be supported by libquvi.
See also C<--format> and L</EXAMPLES>.
...
format was not available. This is determined by libquvi, not cclive.

The only static format strings that libquvi supports are C<default> and
C<best>. They can be used with C<--format>. If I<arg> is C<best> libquvi
0.2.16.1+ will try to determine the C<best> available format to an URL.

If the requested I<arg> format is not available or is unrecognized by
libquvi, it will fall back to C<default>, whatever that may be, and only
if that cannot be found either the library returns an error to cclive.
See also L</EXAMPLES>.

I<arg> C<list> is deprecated and will be removed in the later versions
of cclive. Use C<--query-formats> instead.
...
Whenever possible, get fmt43_360p format of YouTube videos.

=item cclive -p "dailym:best" DAILYMOTION_URL

Similar but get the 'best' format of DailyMotion videos, whenever
possible. The pattern used above is oversimplified. Normally, you
would want to use better crafted patterns, instead, as cclive
does not match the pattern to the domain name but to the I<URL>.

You could also specify multiple occurrences of -p.
