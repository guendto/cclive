...
For reference, see the B<--filename-format> option.

=item cclive -F "%t.%s" -r "(\w+)" URL

Grab the first "word" character from video title and use it in the
output filename.
...
Start playing the video being downloaded with C<mplayer(1)> when
the transfer reaches 20%.

=item echo 'stream-exec = "mplayer -really-quiet %i"' E<gt>E<gt> ~/.ccliverc

=item cclive -s URL

Alternative to streaming with Adobe Flash plugin. C<vlc(1)> and C<totem(1)>
...
Would cause cclive to download mp4 of the Youtube video and vp6_hq of
the Dailymotion video. We make the setting "permanent" by adding it
to ~/.ccliverc.
