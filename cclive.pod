...
Use C<wget(1)> to download the video.
...
Note that libquvi is responsible for maintaining the current
list of the formats. You can get a full list of the hosts
and the supported formats with the C<--hosts>.

The default for all of the supported websites is "flv".
The quality of the default format can vary greatly, and
some hosts have the habit changing the default quality
from time to time. That said, do not read too much
into the details listed below.

Also note that the file suffix is parsed from the content-type
field of the returned HTTP header when the video link is
verified.

All of the above are done by libquvi, not cclive. So if you
see anything funny, please see the L</OTHER> section.
...
Note that if the error message contains "libquvi:", that
means the error occurred in libquvi. Doh. This means that
the bug report should go here:

<http://quvi.googlecode.com/>

Everything else (cclive related) should go here:

<http://cclive.googlecode.com/>

See under the "Issues". Please search the tracker first
before you submit a new one. The issue may have already
been addressed and closed. Thanks.
