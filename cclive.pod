...
retrieved -- or either B<--continue> or  B<--output-video> is used.
...
defaults to "%h_%i.%s". Any of the following specifiers appearing in the
...
Dailymotion video. We make the setting "permanent" by adding it to ~/.ccliverc.
