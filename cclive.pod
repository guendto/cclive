...
Any self-respecting command-line-toolist still using clive, should
consider switching to cclive.
...
=item cclive -f best Youtube_URL
...
=item echo 'format-map = "youtube:best|dailymotion:hq"' E<gt>E<gt> ~/.ccliverc
...
Note that the use of C<--format> overrides --format-map setting.

=back

=head1 FORMATS

cclive (0.6.0+) uses libquvi (<http://quvi.googlecode.com>) to parse
the video links. You can get a complete list of the supported video
websites that libquvi supports with the C<--hosts> option. The
dumped details also include the supported video format IDs that
can be used with the C<--format> option.

Note that cclive no longer uses hardcoded values or keeps track
of the format IDs (or quality, resolution etc., for that matter).
This is a tedious task as some of the supported websites reshuffle
the IDs and other details every now and then. Instead cclive only
dumps whatever libquvi returns and leaves it at that. libquvi
does not go much further than that for the same reasons.

Before you write an email to anyone involved with the project,
consider experimenting with different C<--format> values. The
chances are that no one knows any better, as most websites simply
choose to keep the details unannounced or well buried under tons
of mindnumbing web pages.

Note that "default" format is just a common denominator for whatever
the video website defaults to. The "best" format is usually a some
kind of a compromise unless a proper way to determine the best format
has been found.

If you have additional info about the formats, please report them
to the quvi project <http://quvi.googlecode.com>. This is strictly
(lib)quvi territory.
...
Note that you can also define CCLIVE_HOME.
...
Consider reporting them. Consider even fixing them if you think you
can. Any additional information that you may have been able to dig up
is always welcome and appreciated.

Please, have a look at the already reported (and closed) bugs before
you open another.

If you see "libquvi:" in the error message, this indicates that the
error occurred in libquvi. Those bugs should always be reported to
quvi project's tracker (<http://quvi.googlecode.com/>), and not
cclive's tracker.

Otherwise you can report the bugs to:
...
cclive relies heavily on libquvi for parsing the video links. If
video link parsing breaks, have a look at these files.

For example, you can copy a website script (rule) to  your working
directory (./lua/website/) and tweak it without having to make
any changes to the cclive or libquvi source code.

=item $prefix/share/quvi/docs/howtos

Although these are quvi specific howtos, the guidelines can also
be followed in cclive development.
