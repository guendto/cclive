...
filename if the file exists already, unless:

=over 4

=item * file is already completely retrieved, or:

=item * either --continue --output-video is defined
...
output filenames. Quietly ignored if cclive was built without PCRE.

Refer to PCRE manual page for syntax, patterns etc.
...
defaults to "%t.%s". Any supported specifiers (below) appearing in the
