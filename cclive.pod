...
with video player software, like C<mplayer>, for streaming instead of the Adobe
flash player.
...
cclive appends a numeric suffix (e.g. ".1") to the filename, if the video file
exists already. If the local file length matches the server returned
content-length for the video file, cclive skips the download and ignores any
attempts to resume the transfer.
...
Resume a partially downloaded video file.
...
verifies the video link and exits without downloading the video.
...
filenames. Used in conjunction with the C<--filename-format> option. Refer
to the PCRE manual page for the supported regular expression syntax,
