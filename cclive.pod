...
Evit video details in CSV to stdout. Implies B<--no-extract>.
B<--quiet> will be ignored for CSV output.
...
Identical to the above but tries to work around the (known) SOCKS proxy bug
...
cclive appends a numeric suffix (e.g. ".1") to the output filename
...
Limit download speed to I<amount> (KB/s). Page fetches will
ignore this setting.

=item B<-O, --output-video>=I<file>

Write the downloaded video I<file>. Note that this causes cclive
overwrite existing file without warning.

=item B<-f, --format>=I<format>

Download the I<format> of the video. If set to I<best>, cclive
...
Use the specified I<formatstring> for formatting the output filenames.
cclive defaults to "%t.%s". Any supported specifiers (below) appearing
in the I<formatstring> will be replaced accordingly:
...
Assuming cclive was built --with-pcrecpp.
...
=item * It was written in C and later rewritten in C++
