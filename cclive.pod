...
Run a I<command> subsequently after extracting video. Any "%i" specifiers
in the I<command> are replaced with the extracted video file name.

=item B<-m, --subsequent-mode=>I<MODE>

I<Mode> for running the subsequent I<command>. Possible values are: "sep"
(default) and "all". The I<mode> defines how B<cclive> passes the extracted
video file names to the I<command>. For example:

  % cclive -r "mplayer %i" -m sep URL URL2
  => mplayer VIDEO1
  => mplayer VIDEO2

  % cclive -r "mplayer" -m all URL URL2
...
=head1 EXAMPLES
  ...
  % cclive "http://youtube.com/watch?v=..." -r "ffmpeg -i %i %i.mp3"
