...
cclive is primarily a "video download tool" but it can be used for emitting
parsed video links for other software to use. It can also integrate with video
player software, such as C<mplayer>, making it possible to work around the
...
Print video details in CSV format to stdout. Implies C<--no-extract>.
Ignores C<--quiet> for CSV. Prepends "csv:".
...
a numeric suffix (e.g. ".1") to the filename if it exists already and
starts the download from the beginning -- if it isn't completely
retrieved already.

=item B<-O, --output-video>=I<file>

Write the downloaded video to I<file>. Overwrites the existing file.

=item B<l, --limit-rate>=I<amount>

Limit download speed to I<amount> (KB/s). Ignored for video page fetches.
...
output filenames. Used in conjunction with the C<--filename-format> option.
Refer to the PCRE manual page for the supported regular expression syntax,
patterns, etc. See also L</EXAMPLES>.
...
replacing the "%t". Note the use of "-g" (or C<--find-all>) option.
...
Mimics "streaming" by forking the C<mplayer(1)> when the progress reaches
20% complete. cclive continues to download the video file while C<mplayer(1)>
...
to the source. Serves as a "Adobe flash player" alternative for those who
...
(or C<--stream-exec>) option whenever we want to use mplayer for
...
"flv" is downloaded by default if C<--format> is not specified. Some
...
unicode characters. On a typical Unix-like system, try running "locale -a" to
