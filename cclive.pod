...
I<number> of retries. Specify 0 to disable. Default is 5 times, with
...
(case-insensitive). The default is "/(\w|\s|\pL)/g". See also L</EXAMPLES>.
...
the issue tracker's search feature for this, just be sure to search "All
issues".

If your bug report contains an error message starting with "error: libquvi:",
...
while debugging. Other tools, like C<strace(1)>, C<gdb(1)> and C<valgrind(1)>
