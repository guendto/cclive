...
Ignored if cclive was built WITH_PERL=no.
...
Example:
  % cclive --exec="mplayer -real-quiet %i;" URL
...
Unlike the B<--exec> option which is executed subsequently, B<cclive> forks
the streaming process and continues with the file transfer -- but will not
continue to download another file until the child process terminates.
...
rate drops significantly after starting the process and the forked process
runs out of data, B<cclive> simply ignores this.
...
Extract mp4 format video from the URL:
  % cclive -f mp4 URL

Play extracted video with L<mplayer(1)> subsequently:
...
Start streaming the video being extracted with L<mplayer(1)>
